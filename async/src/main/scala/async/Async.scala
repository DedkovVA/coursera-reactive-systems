...
object Async extends AsyncInterface:
  ...
  def concurrentComputations[A, B](
    ...
    val v1 = makeAsyncComputation1()
    val v2 = makeAsyncComputation2()
    for {
      _v1 <- v1
      _v2 <- v2
    } yield (_v1, _v2)
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    val count = maxAttempts - 1
    makeAsyncComputation().recoverWith {
      case _ if count > 0 => insist(makeAsyncComputation, count)
    }
  }
  ...
  def futurize(callbackBasedApi: CallbackBasedApi): FutureBasedApi =
    val p = Promise[Int]()
    callbackBasedApi.computeIntAsync { (v: Try[Int]) =>
      p.complete(v)
    }
    new FutureBasedApi {
